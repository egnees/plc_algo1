...
ZD_heurist_QAP1::Solution::Solution(int* perm, int obvPerm, int n) : p{perm}, obv{obvPerm}, size{n} {}
...
ZD_heurist_QAP1::List::List() : size{0}, worst{nullptr}, K{0}, a{nullptr} {}
...
// ZD_heurist_QAP1
// constructor(cost, maxListSize)
ZD_heurist_QAP1::ZD_heurist_QAP1(const cost_t& cost, int maxListSize)
        : K{maxListSize}, n{(int) cost.size()}, n2{n * n}, n3{n * n * n},
        n4{n * n * n * n}, d{0}, solutionFactory{n} {
        ...
        for (int j = 0; j < n; ++j) {
            for (int k = 0; k < n; ++k) {
                for (int l = 0; l < n; ++l) {
                    ...
                    // if (cost[i][j][k][l] != cost[j][i][k][l]) {
                    //     throw std::runtime_error("cost i != cost j");
                    // }
                    // if (cost[i][j][l][k] != cost[j][i][l][k]) {
                    //     throw std::runtime_error("cost i != cost j");
                    // }
                    // if (cost[i][j][k][l] != cost[i][j][l][k]) {
                    //     throw std::runtime_error("cost [k][l] != cost [l][k]");
                    // }
                    // if (cost[j][i][k][l] != cost[j][i][l][k]) {
                    //     throw std::runtime_error("cost [k][l] != cost [l][k]");
                    // }
...
std::vector<int> ZD_heurist_QAP1::solve() {
...
int ZD_heurist_QAP1::obv(const int* w) const {
    int ret = 0;
    for (int i = 0; i + 1 < n; ++i) {
        for (int j = i + 1; j < n; ++j) {
...
int* ZD_heurist_QAP1::randPerm() const {
    std::mt19937 rnd(std::chrono::high_resolution_clock::now().time_since_epoch().count());
    int* p = new int[n];
    for (int i = 0; i < n; ++i) {
        p[i] = i;
...
int ZD_heurist_QAP1::deltaObv(int r, int s, const int* w) const {
    int ret = 0;
    for (int i = 0; i < n; ++i) {
        if (i != r && i != s) {
...
ZD_heurist_QAP1::Solution *ZD_heurist_QAP1::SolutionFactory::create(const int* p, int obv) {
    Solution* ret;
    if (!freed.empty()) {
        ret = freed.back();
        freed.pop_back();
